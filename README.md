...
## How to use
Compile a `*.elf` file, the nm tool will automatically update the information. If that did not happen try running the `Nm tool: Run nm` command in vscode. To use the tool with other file extensions you can configure the `nmTool.inputFiles` setting to set a global pattern, the default is `**/*.elf`.
